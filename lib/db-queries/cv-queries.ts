...
export async function getCompleteCV(cvId: string): Promise<CV | null> {
  const cvMeta = await db.execute(
    "SELECT id, user_id, title, status, created_at, updated_at FROM cvs WHERE id = ? LIMIT 1",
    [cvId]
  );
  ...
  const [
    ...
    db.execute(
      "SELECT technical, hard, soft, languages FROM cv_skills WHERE cv_id = ? LIMIT 1",
      [cvId]
    ),
  ...
  const skills: CV["skills"] = {
    technical: s.technical ? JSON.parse(s.technical) : [],
    hard: s.hard ? JSON.parse(s.hard) : [],
    soft: s.soft ? JSON.parse(s.soft) : [],
  ...
  return {
    ...
    personalInfo,
    professionalSummary,
    workExperience, // ✅ 02/2025, 10/2020, 09/2022, 07/2019
